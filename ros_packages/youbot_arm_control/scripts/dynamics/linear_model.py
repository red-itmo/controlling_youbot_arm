...
"""
    ...
    This script makes modules with elements of \xi matrix (regressor).
      Also it may compute expressions for torques, but it something trash :"(
        Also it may compute M(q), C(q, dq) and G(q) matrices,
...
# !dont remember where need add theta_2 (default)
def T(i, j, simp=False):
    ...
    if Ts[i][j] != -1:
        ...
        for k in range(i, j):
            ...
            Rx = Matrix([[1, 0, 0, 0], [0, cos(alphai[i]), -sin(alphai[i]), 0], [0, sin(alphai[i]), cos(alphai[i]), 0],
                         [0, 0, 0, 1]])
